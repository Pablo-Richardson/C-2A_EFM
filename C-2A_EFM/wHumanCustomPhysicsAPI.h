...
This header file defines the custom physics API for integrating a flight model (FM) with the DCS (Digital Combat Simulator) engine. It’s meant for developers writing advanced aircraft simulations, letting their flight model code communicate with the simulator about forces, moments, environment, mass, damage, fuel, and events.
Almost everything is defined as a function pointer type—basically, a way for DCS to call back into your C++ code for simulation logic.

The file is organized into logical sections:

Force & Moment application (how your FM provides physical forces to DCS)
Simulation lifecycle hooks (functions you implement to update state each frame)
Environmental data (wind, surface, atmosphere)
Aircraft state and mass management
Fuel handling
Rendering and visual arguments
Parameter querying (engine data, control positions, etc.)
Initialization and configuration
Damage, failures, and repair
Event communication (both ways between DCS and your FM)
Special effects (LERX vortex, suspension feedback)
Debugging and plugin paths
If you’re writing a custom flight model for DCS, this file is your “contract” for everything your code needs to implement or understand.

Essential C++ Skills Before Diving In
Pointers and References
Function pointers: How to declare, assign, and use them.
References: Especially double &x style, for in/out function arguments.
Structs and Classes
Declaring, initializing, and using C++ structs and classes.
Constructors, member variables, and default values.
Enums (including enum class)
Using enum and enum class for type-safe enumerations.
Memory Management
Allocation and deallocation, especially for arrays of structs (e.g., wind fields).
Understanding nullptr, pointer arithmetic, and sizeof.
Function Typedefs / using
Typedef/using for function pointer types, e.g. typedef void (*MyFunc)(int);
C/C++ Interop
extern "C" and ABI compatibility (not shown in this file, but common in plugin APIs).
Callbacks and API Contracts
How to implement functions that will be called by an external engine (DCS).
Basic C++ Syntax
Comments, function prototypes, unions, casting, etc.
Handling Arrays and Loops
Especially for wind fields and event processing.
Understanding the Build Process
How to compile and link code for use as a plugin or shared library.
If you’re not comfortable with these, especially pointers/references and function pointers, review them before implementing your own FM.

Section-by-Section Breakdown
1. Force and Moment Application
PFN_FORCE_SOURCE and PFN_FORCE_COMPONENT_SOURCE

Functions to provide force vectors (and their positions) in body coordinates.
Component version returns bool—called repeatedly until false.
PFN_MOMENT_SOURCE and PFN_MOMENT_COMPONENT_SOURCE

Functions to provide moment (torque) vectors in body axes.
Component version is called repeatedly like above.
2. Simulation Lifecycle Functions
PFN_SIMULATE

Called every simulation step, you update FM state here.
PFN_SET_SURFACE

Called to set up the ground/surface info before simulation step.
PFN_SET_ATMOSPHERE

Called to set up atmospheric conditions (altitude, temp, wind, etc.).
3. Wind Field Interface
WNDVF_SPACE enum

Whether wind data is expected in body or world space.
wind_vector_field_component & wind_vector_field

Structures for specifying wind at multiple points.
PFN_WIND_VECTOR_FIELD_UPDATE_REQUEST / PFN_WIND_VECTOR_FIELD_DONE

Process for updating wind fields—DCS calls you, you fill in the data.
4. Mass and State Management
PFN_CURRENT_MASS_STATE

Set mass and moments of inertia.
PFN_CURRENT_STATE / PFN_CURRENT_STATE_BODY_AXIS

Set the aircraft’s position, orientation, velocity, etc., in world or body axes.
PFN_SET_COMMAND

Passes cockpit/control commands to your FM.
5. Mass Changes & Fuel Handling
PFN_CHANGE_MASS

Called to apply incremental mass changes (e.g., dropping stores, burning fuel).
PFN_SET_INTERNAL_FUEL / PFN_GET_INTERNAL_FUEL

Manage internal fuel volume.
PFN_SET_EXTERNAL_FUEL / PFN_GET_EXTERNAL_FUEL

Manage external fuel (e.g., drop tanks).
PFN_REFUELING_ADD_FUEL

For incremental refueling events.
6. Drawing/Rendering Arguments
EdDrawArgument struct

Union for draw arguments (float, pointer, int).
PFN_SET_DRAW_ARGS

Update visual arguments (e.g., for cockpit animations).
7. Parameter Querying
ed_fm_param_enum

Huge enum for all possible FM parameters (engine data, suspension, cockpit controls, etc.).
PFN_GET_PARAM

Called to fetch a specific parameter’s current value.
8. FM Initialization and Startup
PFN_COLD_START / PFN_HOT_START / PFN_HOT_START_IN_AIR

Prepare FM for cold, hot, or in-air start.
PFN_MAKE_BALANCE

(Advanced) Called to balance FM at specific speed/height.
9. Debug, Configuration, and Plugin Path
PFN_ENABLE_DEBUG_INFO

Enable extra debug output.
PFN_DEBUG_WATCH

Output formatted debug info for DCS overlay.
PFN_SET_PLUGIN_DATA_INSTALL_PATH

Communicate plugin install path to DCS.
10. Damage, Failure, and Repair
PFN_ON_PLANNED_FAILURE / PFN_ON_DAMAGE / PFN_REPAIR / PFN_NEED_TO_BE_REPAIRED

Hooks for handling and recovering from damage and failures.
PFN_FM_SET_IMMORTAL / PFN_FM_SET_UNLIMITED_FUEL / PFN_FM_SET_EASY_FLIGHT

Set special modes (invulnerability, unlimited fuel, easy FM).
PFN_FM_SET_PROPERTY_NUMERIC / PFN_FM_SET_PROPERTY_STRING

Set arbitrary custom FM properties.
11. Simulation Events Interface
ed_fm_simulation_event / ed_fm_simulation_event_type

Structures/enums for simulation events (failures, fire, carrier ops).
PFN_FM_POP_SIMULATION_EVENT / PFN_FM_PUSH_SIMULATION_EVENT

Exchange events between DCS and your FM.
12. Suspension, Feedback, and Effects
ed_fm_suspension_info / PFN_FM_SUSPENSION_FEEDBACK

Feedback for gear suspension state.
LERX_vortex_spline_point / LERX_vortex / PFN_FM_LERX_VORTEX_UPDATE

Special effect for LERX vortex visualization.
TL;DR
This file is the API contract between your custom FM and the DCS engine.
It’s heavily reliant on function pointers, structs, and enums.
You must know how to work with C++ pointers, references, function pointers, structs, memory, and callbacks.
If you want a walkthrough of a specific section, a deep dive into one function type, or a crash course in any essential C++ topic, let me know!
*/
